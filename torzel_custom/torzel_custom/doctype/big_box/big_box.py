...
class BigBox(Document):
    ...
    def validate(self):
        ...
        3) Barcode must be Submitted (not Cancelled) in the linked DocType.
        4) Barcode must not already be delivered (DN/SI) or present in another Big Box (Draft/Submitted).
        """
        ...
        for d in (self.barcode_list or []):
            ...
            # Require the linked record to be Submitted (docstatus=1) and not Cancelled (docstatus=2)
